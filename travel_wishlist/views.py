...
def place_detail(request, pk):
    ...
    visited = get_object_or_404(Places, pk=pk)
    if request.method == "POST":
        form = NewPlaceForm(request.POST, instance=visited)
        if form.is_valid():
            post = form.save(commit=False)
            post.save()
            return redirect('place_detail', pk=post.pk)
    else:
        form = NewPlaceForm(instance=visited)
  
    return render(request, 'travel_wishlist/place_details.html', {'visited': visited, 'form': form})
